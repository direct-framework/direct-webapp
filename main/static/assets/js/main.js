var main = (function () {
  ...
  */
  ...
  const catAnnotationPointOuterLabel = (categoryStartAngleMap, catLabelWidthHeightMap, xPadding, yPadding, plotHeight) => categoryId => {
    ...
    const xSide = Math.sin(angle) > 0 ? 1 : -1;
    const ySide = -Math.cos(angle) > 0 ? 1 : -1;
    const catsInThisQuadrant = Object.keys(categoryStartAngleMap).filter(catId => {
      const inXSide = Math.sin(categoryStartAngleMap[catId]) > 0 ? 1 : -1;
      const inYSide = -Math.cos(categoryStartAngleMap[catId]) > 0 ? 1 : -1;
      return inXSide === xSide && inYSide === ySide;
    ...
    const thisCatIndex = catsInThisQuadrant.length - catsInThisQuadrant.indexOf(categoryId) - 1;
    const preCatHeights = catsInThisQuadrant.slice(0, thisCatIndex).reduce((acc, catId) => acc + catLabelWidthHeightMap[catId].height, 0);
  ...
  */
  ...
  function renderAnnotationsD3({
    ...
    const lvlsAnnotationContainer = annotationContainer.append('g').attr('class', 'annotation-lvls-container');
    if (lvlLabelType !== 'none') lvlsArray.forEach(lvl => {
      lvlsAnnotationContainer.append('text').attr('class', 'annontation-lvl').attr('x', 0).attr('y', -getYPoint(lvl.level + 1) + fontSize / 2) // level + 1 as we want this at the top of the level
      .attr('fill', lvlTextColor).attr('text-anchor', 'middle').attr('font-size', fontSize).text(lvl.name);
    });

    /* Category Annotations */
    filteredCategories.forEach(cat => {
      const categoryAnnotationContainer = annotationContainer.append('g').attr('class', `annotation-cat annotation-${cat.id.replaceAll(' ', '-')}`).attr('fill', cat.color);
      ...
      const categoryAnnotationLine = categoryAnnotationContainer.append('g').attr('class', 'annotation-cat-line');

      // Arc at base of category
      categoryAnnotationLine.append('path').attr('d', categoryBaseArc(cat.id)).attr('fill', cat.color).attr('stroke', 'none').attr('stroke-width', lineThickness);

      // Line from base of category to outer radius
      categoryAnnotationLine.append('line').attr('x1', baseOfCategoryAnchor.x).attr('y1', baseOfCategoryAnchor.y).attr('x2', outerAnnotationLineAnchor.x).attr('y2', outerAnnotationLineAnchor.y).attr('stroke', cat.color).attr('fill', 'none').attr('stroke-width', lineThickness).attr('opacity', 1);

      // Line from outer radius to category label
      categoryAnnotationLine.append('line').attr('x1', outerAnnotationLineAnchor.x).attr('y1', outerAnnotationLineAnchor.y).attr('x2', labelAnchorPoint.x + (labelXDir > 0 ? 0 : labelWidth)).attr('y2', labelAnchorPoint.y).attr('stroke', cat.color).attr('fill', 'none').attr('stroke-width', lineThickness).attr('opacity', 1);

      // Line beneath category label
      categoryAnnotationContainer.append('line').attr('x1', labelAnchorPoint.x).attr('y1', labelAnchorPoint.y).attr('x2', labelAnchorPoint.x + labelWidth).attr('y2', labelAnchorPoint.y).attr('stroke', cat.color).attr('fill', 'none').attr('stroke-width', lineThickness);

      // Category label text box
      categoryAnnotationContainer.append('rect').attr('class', 'label-box').attr('x', labelAnchorPoint.x).attr('y', labelAnchorPoint.y + (labelYDir === 1 ? 0 : -labelHeight)).attr('width', labelWidth).attr('height', labelHeight).attr('color', labelTextColor).attr('opacity', 0.8) // increased on hover
      .attr('fill', cat.color);
      const highlightLineThickness = 10;
      // Line On Side of category label
      categoryAnnotationContainer.append('line').attr('class', 'highlight-line').attr('x1', labelAnchorPoint.x + labelXDir * highlightLineThickness + (labelXDir === 1 ? labelWidth : 0)).attr('y1', labelAnchorPoint.y).attr('x2', labelAnchorPoint.x + labelXDir * highlightLineThickness + (labelXDir === 1 ? labelWidth : 0)).attr('y2', labelAnchorPoint.y + labelHeight * labelYDir).attr('stroke', cat.color).attr('opacity', 0) // only shown on hover
      ...
      labelDirected.forEach((line, i) => {
        categoryAnnotationContainer.append('text').attr('x', labelAnchorPoint.x + labelWidth / 2).attr('y', labelAnchorPoint.y + i * fontSize * labelYDir + fontSize / 2 * labelYDir).attr('class', 'annotation-cat-label').attr('fill', labelTextColor).attr('font-weight', 700).attr('font-size', fontSize).attr('text-anchor', 'middle').attr('dominant-baseline', 'middle').attr('color', labelTextColor).text(line);
      });
    });
    return svg;
  }
  ...
  function RadialBarChart({
    ...
    function refreshSkillHighlightD3(svg, highlightedSkill, innerCircleWidth, fontSize) {
      ...
      const t = transition().delay(200 * transitionMultiplier).duration(200 * transitionMultiplier).ease(linear$1);
      if (!highlightedSkill) {
        ...
        group.select('.skill-highlight-text-skill').selectAll('skill-highlight-text-skill-central').data(skillTextSplit).enter().append('text').attr('font-size', fontSize).attr('x', 0).attr('y', (_, i) => -(0.5 * fontSize * skillTextSplit.length / 2) + i * fontSize * 1.2).attr('class', 'skill-highlight-text-skill-central').attr('text-anchor', 'middle').attr('fill', labelTextColor).text(skillText => skillText);
    ...
    function renderBackgroundLvlRingsD3(svg) {
      ...
      lvlsArray.forEach(lvl => {
        ringGroup.append('path').attr('class', 'lvl-rings-ring').attr('d', lvlRing(lvl.level)).attr('fill', lvlArcColor).attr('stroke', 'none').attr('stroke-width', 1);
      });
      return svg;
    }
